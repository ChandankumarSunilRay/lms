...
const Lecture = ({ user }) => {
  ...
  const [lecture, setLecture] = useState(null); // initially no lecture
  ...
  async function fetchLectures() {
    try {
      ...
      // ✅ Automatically load & play the first lecture when page loads
      if (data.lectures.length > 0) {
        fetchLecture(data.lectures[0]._id); // fetch the first lecture
      }
    } catch (error) {
      setLoading(false);
    }
  ...
  async function fetchLecture(id) {
    ...
    try {
      ...
      // ✅ Autoplay video once lecture is loaded
      setTimeout(() => {
        if (videoRef.current) {
